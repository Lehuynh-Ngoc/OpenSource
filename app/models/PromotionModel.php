...
class PromotionModel extends Database {
    ...
    public function getAll() {
        $query = "SELECT p.*, 
                  CASE 
                    WHEN p.target_id = 0 THEN 'Tất cả'
    ...
    public function update($id, $name, $type, $target_id, $discount_type, $discount_value, $start_date, $end_date) {
        $query = "UPDATE promotions SET name = :name, type = :type, target_id = :target_id, 
    ...
    public function getActivePromotionForProduct($productId, $categoryId) {
        // Ưu tiên: Sản phẩm cụ thể -> Danh mục cụ thể -> Tất cả (target_id = 0)
        ...
        $query = "SELECT * FROM promotions WHERE status = 1 
                  ...
                  AND (
                      (type = 'product' AND (target_id = :p_id OR target_id = 0)) 
                      OR (type = 'category' AND (target_id = :c_id OR target_id = 0))
                  )
                  ORDER BY (CASE WHEN target_id != 0 THEN 0 ELSE 1 END) ASC, type ASC LIMIT 1";
        $stmt = $this->db->prepare($query);
